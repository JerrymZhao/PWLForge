`ifndef gelu_CONFIG_VH
`define gelu_CONFIG_VH
...
`define OPT_TOTAL_INTERVALS 88
`define OPT_MAX_INTERVALS_PER_GROUP 20
...
`define OPT_INTERVAL_METADATA_SIZE 264  // start[1] + end[1] + gid[1]
`define OPT_GROUP_BOUNDS_SIZE 14  // min[1] + max[1]
`define OPT_GROUP_MAP_SIZE 7
`define OPT_GROUP_INFO_SIZE 21  // flags[1] + b[1] + c[1]
`define OPT_DELTA_DATA_SIZE 264  // delta_b[1] + delta_c[1] + flags[1]

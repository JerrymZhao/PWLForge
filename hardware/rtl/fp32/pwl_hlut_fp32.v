...
module pwl_hlut_fp32 (
    ...
    function [31:0] combine_fp32;
        input [15:0] high, low;
        begin
            combine_fp32 = {high, low};
        end
    endfunction
    ...
    function [31:0] negate_fp32;
        input [31:0] fp;
        begin
            negate_fp32 = {~fp[31], fp[30:0]};
        end
    endfunction
    
    function [31:0] abs_fp32;
        input [31:0] fp;
        begin
            abs_fp32 = {1'b0, fp[30:0]};
        end
    endfunction
    ...
    generate
        for (gi = 0; gi < TOTAL_INTERVALS; gi = gi + 1) begin : gen_intervals
            localparam BASE = gi * 5;
            assign interval_start[gi] = combine_fp32(
                interval_metadata[BASE+0], interval_metadata[BASE+1]);
            assign interval_end[gi] = combine_fp32(
                interval_metadata[BASE+2], interval_metadata[BASE+3]);
            assign interval_to_group[gi] = interval_metadata[BASE+4][GROUP_ADDR_WIDTH-1:0];
        end
    endgenerate
    ...
    assign abs_x_s0 = abs_fp32(s_axis_x_tdata);
    assign valid_s0 = s_axis_x_tvalid;
    assign s_axis_x_tready = 1'b1;
    
    //==========================================================================
    // Stage 1-2: Comparators (2 cycle latency) - UPDATED INSTANTIATION
    //==========================================================================
    ...
    generate
        for (gi = 0; gi < TOTAL_INTERVALS; gi = gi + 1) begin : gen_cmp
            fp_compare_ge_wrapper #(
                .DATA_WIDTH(32), 
                .METADATA_WIDTH(1)  // Dummy metadata, not used
            ) cmp_start (
                .aclk(aclk), 
                .aresetn(aresetn),
                // Operand A
                .s_axis_a_tdata(abs_x_s0), 
                .s_axis_a_tvalid(valid_s0), 
                .s_axis_a_tready(),
                // Operand B
                .s_axis_b_tdata(interval_start[gi]), 
                .s_axis_b_tvalid(valid_s0), 
                .s_axis_b_tready(),
                // Result
                .m_axis_result_tdata(ge_start[gi]), 
                .m_axis_result_tdata_full(),
                .m_axis_result_tvalid(ge_start_valid[gi]), 
                .m_axis_result_tready(1'b1),
                // Metadata (unused)
                .s_axis_metadata_tdata(1'b0), 
                .s_axis_metadata_tvalid(1'b0), 
                .s_axis_metadata_tready(),
                .m_axis_metadata_tdata(), 
                .m_axis_metadata_tvalid(), 
                .m_axis_metadata_tready(1'b1)
            );
            
            fp_compare_lt_wrapper #(
                .DATA_WIDTH(32), 
                .METADATA_WIDTH(1)  // Dummy metadata, not used
            ) cmp_end (
                .aclk(aclk), 
                ...
                .s_axis_a_tready(),
                // Operand B
                .s_axis_b_tdata(interval_end[gi]), 
                .s_axis_b_tvalid(valid_s0), 
                .s_axis_b_tready(),
                // Result
                .m_axis_result_tdata(lt_end[gi]), 
                .m_axis_result_tdata_full(),
                .m_axis_result_tvalid(lt_end_valid[gi]), 
                .m_axis_result_tready(1'b1),
                // Metadata (unused)
                .s_axis_metadata_tdata(1'b0), 
                .s_axis_metadata_tvalid(1'b0), 
    ...
    simple_delay_line #(.DATA_WIDTH(32), .DEPTH(2)) delay_abs_x (
    ...
    reg [31:0] base_b_comb, base_c_comb, delta_b_comb, delta_c_comb;
    reg is_orphan_comb, reflection_x_comb, reflection_y_comb;
    
    always @(*) begin
        base_b_comb = 32'h00000000;
        base_c_comb = 32'h00000000;
        delta_b_comb = 32'h00000000;
        delta_c_comb = 32'h00000000;
        is_orphan_comb = 1'b1;
        ...
        if (match_found_s3) begin
            ...
            grp_base = storage_idx * 5;
            delta_base = found_interval_s3 * 5;
            
            flags = group_info[grp_base + 0];
            is_orphan_comb = flags[0];
            
            base_b_comb = combine_fp32(group_info[grp_base+1], group_info[grp_base+2]);
            base_c_comb = combine_fp32(group_info[grp_base+3], group_info[grp_base+4]);
            delta_b_comb = combine_fp32(delta_data[delta_base+0], delta_data[delta_base+1]);
            delta_c_comb = combine_fp32(delta_data[delta_base+2], delta_data[delta_base+3]);
            
            delta_flags = delta_data[delta_base + 4];
    ...
    reg [31:0] abs_x_s4;
    reg valid_s4;
    reg [31:0] base_b_s4, base_c_s4, delta_b_s4, delta_c_s4;
    reg is_orphan_s4;
    ...
    //==========================================================================
    localparam RECON_META_WIDTH = 36;
    wire [RECON_META_WIDTH-1:0] recon_meta_in;
    assign recon_meta_in = {abs_x_s4, x_sign_s4, reflection_x_s4, reflection_y_s4, 1'b0};

    wire [31:0] slope_a_input = is_orphan_s4 ? delta_b_s4 : base_b_s4;
    wire [31:0] slope_b_input = is_orphan_s4 ? 32'h00000000 : delta_b_s4;
    wire [31:0] intercept_a_input = is_orphan_s4 ? delta_c_s4 : base_c_s4;
    wire [31:0] intercept_b_input = is_orphan_s4 ? 32'h00000000 : delta_c_s4;

    wire [31:0] slope_recon, intercept_recon;
    wire slope_recon_valid, intercept_recon_valid;
    wire [RECON_META_WIDTH-1:0] slope_meta_out, intercept_meta_out;

    fp_add #(.DATA_WIDTH(32), .METADATA_WIDTH(RECON_META_WIDTH)) slope_adder (
    ...
    fp_add #(.DATA_WIDTH(32), .METADATA_WIDTH(RECON_META_WIDTH)) intercept_adder (
        .aclk(aclk), .aresetn(aresetn),
    ...
    //==========================================================================
    reg [31:0] slope_s5, intercept_s5, abs_x_s5;
    ...
    // Stage 6-7: Multiply
    //==========================================================================
    wire [31:0] x_for_mult;
    assign x_for_mult = reflection_x_s5 ? negate_fp32(abs_x_s5) : abs_x_s5;
    
    localparam MULT_META_WIDTH = 34;
    ...
    fp_mult #(.DATA_WIDTH(32), .METADATA_WIDTH(MULT_META_WIDTH)) multiplier (
        .aclk(aclk), .aresetn(aresetn),
    ...
    fp_add #(.DATA_WIDTH(32), .METADATA_WIDTH(FINAL_META_WIDTH)) final_adder (
        .aclk(aclk), .aresetn(aresetn),
        .s_axis_a_tdata(mult_result), .s_axis_a_tvalid(mult_valid), .s_axis_a_tready(),
        .s_axis_b_tdata(mult_meta_out[33:2]), .s_axis_b_tvalid(mult_valid), .s_axis_b_tready(),
    ...
    // Output reflection
    //==========================================================================
    wire [31:0] reflected_y, final_y;
    
    assign reflected_y = final_meta_out[0] ? negate_fp32(add_result) : add_result;
    assign final_y = final_meta_out[1] ? negate_fp32(reflected_y) : reflected_y;
...
// Helper modules
//==============================================================================
module simple_delay_line #(parameter DATA_WIDTH = 32, parameter DEPTH = 10)(
